...
class LazyBlocks_3rd_Astra {
    ...
    public function constructor_enqueue_scripts() {
        if ( 'lazyblocks' === get_post_type() ) {
            wp_enqueue_style( 'lazyblocks-constructor-3rd-astra', lazyblocks()->plugin_url() . 'dist/assets/editor-constructor/3rd/astra.min.css', array(), LAZY_BLOCKS_VERSION );
            wp_style_add_data( 'lazyblocks-constructor-3rd-astra', 'rtl', 'replace' );
            wp_style_add_data( 'lazyblocks-constructor-3rd-astra', 'suffix', '.min' );
