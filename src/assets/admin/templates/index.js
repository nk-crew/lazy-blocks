...
const {
    used_post_types_for_templates: usedPostTypes,
...
class UpdateEditor extends Component {
    ...
    render() {
        ...
        postTypes.filter( ( post ) => post.viewable && 'lazyblocks' !== post.slug && 'lazyblocks_templates' !== post.slug && 'attachment' !== post.slug ).forEach( ( post ) => {
            let { label } = post;
