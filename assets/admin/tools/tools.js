...
const { navigator, lazyblocksToolsData: data } = window;
...
export default function Templates() {
	...
	function renderExportContent(type = 'blocks') {
		...
		const nothingSelected =
			Object.keys(disabledStates[`disabled${typeLabel}`]).length ===
			data[type].length;
							...
							return (
								<ToggleControl
									...
									onChange={() => {
										...
										if (
											isSelected &&
											!newDisabled[item.data.id]
										) {
											newDisabled[item.data.id] = true;
										} else if (
											!isSelected &&
											typeof newDisabled[item.data.id] !==
												'undefined'
										) {
					...
					<div className="postbox-container">
						<div className="postbox">
							...
							{data.blocks && data.blocks.length ? (
								...
								<div className="inside">
									<p>
										{__('There are no blocks to export.')}
									</p>
								</div>
							)}
						</div>
							...
							<div className="inside">
								...
								>
									<div className="lzb-export-select-items">
										<input
											type="file"
											name="lzb_tools_import_json"
										/>
									</div>

									<input
										type="hidden"
										name="lzb_tools_import_nonce"
										value={data.nonce}
									/>
